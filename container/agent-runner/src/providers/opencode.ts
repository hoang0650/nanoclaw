...
export function buildOpenCodeConfig(options: ProviderOptions): Record<string, unknown> {
  ...
  // Prefer file-mounted key (Aimarkets / gateway=none). Fall back to OneCLI
  // placeholder so HTTPS_PROXY can overwrite on the wire.
  let apiKey = 'placeholder';
  const keyFile = process.env.OPENCODE_API_KEY_FILE?.trim();
  if (keyFile && existsSync(keyFile)) {
    try {
      const fromFile = readFileSync(keyFile, 'utf8').trim();
      if (fromFile) apiKey = fromFile;
    } catch (err) {
      log(`Failed to read OPENCODE_API_KEY_FILE: ${err instanceof Error ? err.message : String(err)}`);
    }
  }

  // Featherless is OpenAI-compatible; reuse the openai-compatible transport.
  const usesOpenAICompatible = provider === 'openai' || provider === 'featherless';
  ...
  const providerOptions: Record<string, unknown> =
    provider === 'anthropic'
      ...
      : {
          [provider]: {
            // A custom base URL on the `openai` / `featherless` provider means a
            // self-hosted or third-party OpenAI-compatible endpoint. The stock
            // openai SDK package speaks the Responses API — pin Chat Completions.
            // Scoped away from `openrouter`, which ships its own ai-sdk package.
            ...(usesOpenAICompatible && proxyUrl ? { npm: '@ai-sdk/openai-compatible' } : {}),
